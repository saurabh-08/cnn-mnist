...
def run_main(FLAGS):
    ...
    print(f"Starting Model: {FLAGS.mode}\n")
...
#     optimizer = NotImplementedError()
    ...
    # Lists to store losses and accuracies for each epoch for plotting    
    train_losses = []     
    train_accuracies = []     
    test_losses = []     
    test_accuracies = []
    ...
    # Plotting the graphs     
    epochs = list(range(1, FLAGS.num_epochs + 1))     
    ...
    plt.ylabel("Accuracy")     
    plt.legend()      
    plot_filename = f'training_results_mode_{FLAGS.mode}.png'          
    plt.tight_layout()     
    plt.savefig(os.path.join(FLAGS.log_dir, plot_filename))     
    plt.show()
...
if __name__ == '__main__':
    ...
    # Epoch modification for model 5   
    if FLAGS.mode == 5:         
        FLAGS.num_epochs = 40
